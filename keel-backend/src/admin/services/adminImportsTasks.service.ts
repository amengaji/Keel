...
export async function buildTaskImportTemplateXlsxBuffer(): Promise<Buffer> {
  ...
  // Validation Dropdown for Ship Types
  if (shipTypeNames.length > 0) {
    // Hidden sheet for list
    ...
    // Apply to column G (Ship Type) rows 2-1000
...
export async function previewTaskImportXlsx(
  ...
  // Load Ship Types for validation
  const shipTypes = await ShipType.findAll();
  const shipTypeMap = new Map<string, number>(); // Name -> ID
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const issues: string[] = [];
    if (!part_number) issues.push("part_number is required and must be a number");
    ...
    // TODO: Add Duplicate Check (Title + ShipTypeID) against DB here if needed
    // For now, we rely on Commit-phase unique constraints or checks.
